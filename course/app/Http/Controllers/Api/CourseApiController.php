...
class CourseApiController extends Controller
{
    ...
    public function getAllCourses(Request $request)
    {
        ...
        $courses = $query->where('type', '1')->with('user')->where('status', 1)->where('scope', 1)->latest()->get();
    ...
    public function getAllClasses(Request $request)
    {
        ...
        $courses = $query->where('type', '3')->with('user', 'class')->where('scope', 1)->where('status', 1)->latest()->get();
        $response = [
            'success' => true,
            'data' => $courses,
            'total' => count($courses),
    ...
    public function getQuizDetails($id)
    {
        ...
        $user = auth('api')->user();
        $course = Course::with($relation)->findOrFail($id);
        if ($user != null) {
            $course->isEnrolled = !!$course->isLoginUserEnrolled;
        }
    ...
    public function getLessonQuizDetails($id)
    {
        ...
        $relation = ['assign', 'assign.questionBank', 'assign.questionBank.questionMu', 'assign.questionBank.questionMuInSerial'];
