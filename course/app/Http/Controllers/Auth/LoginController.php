    ...
    public function login(Request $request)
    {
        ...
        if (isModuleActive('TwoFA') && Settings('enable_two_fa')) {
            ...
            if ($request->two_step_verify_code) {
                $requested_user = UserOtpCode::where('email', $request->email)->latest()->first();
                if ($requested_user) {
                    $different = $requested_user->created_at->diffInMinutes(Carbon::now());

                    if ($requested_user->otp_code != $request->two_step_verify_code) {
                        Toastr::error(trans('auth.Your verification code does not matched'), trans('common.Failed'));
                        return back();
                    } elseif ($different > $target_user->two_fa_expired_time) {
            ...
            } elseif ($target_user && ($target_user->two_step_verification == 1 || $target_user->two_step_verification == 3 || $target_user->two_step_verification == 4)) {
                $user = User::where('email', $request->email)->first();
                \session()->put('email', $request->email);
                \session()->put('password', $request->password);
                \session()->put('force', $request->force);
                $userRepository = App::make(UserRepositoryInterface::class);
                $verification_code = $userRepository->storeVerificationCode($request->email);
                ...
                if ($verification_code) {
                    ...
                    if ($target_user->two_step_verification == 1) {
                        send_email($user, 'Two_Factor_Authentication', $data);
                    } elseif ($target_user->two_step_verification == 3) {
                        send_sms_notification($user, 'Two_Factor_Authentication', $data);
                    } elseif ($target_user->two_step_verification == 4) {
