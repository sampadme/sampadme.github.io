    ...
    public function toArray(Request $request): array
    {
        ...
        foreach ($this->files as $file) {
            $files[] = [
                ...
                'file'      => file_exists($file->file) ? (string)asset($file->file) : '',
                'lock'      => (int)$file->lock,
        ...
        if (isModuleActive('EarlyBird')) {
            $pricePlans = [];
            foreach ($this->pricePlans ?? [] as $pricePlan) {
                $pricePlans[] = [
                    'id'        => (int)$pricePlan->id,
                    'course_id' => (int)$pricePlan->price_planable_id,
                    'tile'      => (string)$pricePlan->title,
                    'discount'  => (float)$pricePlan->discount_amount,
                    'date'      => (string)"$pricePlan->start_date - $pricePlan->end_date",
                ];
            }
        } else {
            $pricePlans = null;
        ...
        return [
            ...
            'image'                         => file_exists($this->image) ? (string)asset($this->image) : '',
            'thumbnail'                     => file_exists($this->thumbnail) ? (string)asset($this->thumbnail) : '',
