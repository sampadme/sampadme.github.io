...
//        }
    ...
    public function getIsLoginUserEnrolledAttribute()
    {
        ...
        if (\auth()->user()->role_id == 2) {
            if ($this->user_id == \auth()->user()->id) {
                ...
                return true;
            } elseif (!empty($this->assistant_instructors) && in_array(\auth()->user()->id, $this->assistantInstructorsIds)) {
                return true;
            } elseif ($this->enrollUsers->where('id', \auth()->user()->id)->count()) {
                return true;
            }
        }
    ...
    {
        return $this->morphMany(\Modules\EarlyBird\Entities\PricePlan::class, 'price_planable');
    }
