    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isModuleActive('OrgSubscription')) {
                ...
                $autoEnrollment->autoEnrollment($user);
            }
            Session::forget('type');
...
//                return $query->name;
        ...
        try {
            ...
            foreach ($students as $student) {
                ...
                if ($user) {
                    ...
                    if ($check) {
                        Toastr::error($user->name . ' '.trans('frontend.has already been enrolled to this course'), trans('common.Success'));
                        ...
                        Toastr::success($user->name . ' '.trans('frontend.Successfully Enrolled this course'), trans('common.Success'));
                    }
    ...
    public function Skill_group($id)
    {
        if (isModuleActive('SkillAndPathway')) {
            $groups = GroupStudent::where('student_id', $id)->with('group')->get();
